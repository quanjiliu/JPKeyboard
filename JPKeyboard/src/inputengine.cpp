...
public:
        ...
        recursiveMethodLock(0)
    {
        if (inputMethod == NULL)
        {
            //inputMethod = new PlainInputMethod();
            //inputMethod = new PinyinInputMethod();
        }
    }
...
void InputEngine::setInputMethod(AbstractInputMethod *inputMethod)
{
    ...
    if (d->inputMethod != inputMethod) {
        ...
        if (d->inputMethod) {
            ...
            for (int i = 0; i < activeSelectionLists.size(); i++) {
                const SelectionListModel::Type &selectionListType = activeSelectionLists.at(i);
                QMap<SelectionListModel::Type, SelectionListModel *>::iterator it = d->selectionListModels.find(selectionListType);
            ...
            // Deallocate inactive selection lists
             for (int i = 0; i < inactiveSelectionLists.size(); i++) {
                const SelectionListModel::Type &selectionListType = (inactiveSelectionLists).at(i);
                QMap<SelectionListModel::Type, SelectionListModel *>::const_iterator it = d->selectionListModels.constFind(selectionListType);
...
QList<int> InputEngine::inputModes() const
{
    ...
    for (int i = 0; i < inputModeList.size(); i++)
    {
        const InputMode &inputMode = (inputModeList).at(i);
...
{
    Q_D(const InputEngine);
    QMap<SelectionListModel::Type, SelectionListModel *>::const_iterator it = d->selectionListModels.constFind(SelectionListModel::WordCandidateList);
    if (it == d->selectionListModels.cend())
        return false;
...
QList<int> InputEngine::patternRecognitionModes() const
{
    ...
    for (int i = 0 ; i < patterRecognitionModeList.size(); i++)
    {
        const PatternRecognitionMode &patternRecognitionMode = (patterRecognitionModeList).at(i);
