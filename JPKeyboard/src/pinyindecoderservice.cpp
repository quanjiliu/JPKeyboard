...
bool PinyinDecoderService::init()
{
    ...
    QString sysDict = strDir + "/data/pinyin/dict_pinyin.dat";
    QString usrDictPath = strDir + "/data/pinyin/usr_dict.dat";

    if (QFileInfo::exists(sysDict) == false)
    {
        VIRTUALKEYBOARD_DEBUG() << "data/pinyin/dict_pinyin.dat not found" << sysDict;
        return false;
    }

    QFileInfo usrDictInfo(usrDictPath);
